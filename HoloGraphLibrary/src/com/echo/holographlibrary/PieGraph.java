    ...
    public void onDraw(Canvas canvas) {
        ...
        for (PieSlice slice : mSlices) {
            ...
            if (!TextUtils.isEmpty(title)) {
                ...
                mPaint.getTextBounds(title, 0, length, bounds);
                int textWidth = bounds.width();
                int textHeight = bounds.height();
                float vOffset = (radius - innerRadius + textHeight) / 2;
                float arcLength = (float)(2 * radius * 3.14f * (slice.getValue()/totalValue) - radius * Math.tan((double)(mPadding/2f * 0.017f)) * 2f);
                if (textWidth > arcLength * 0.9) {
                    float multiplier = textWidth / (arcLength * 0.9f);
                    textWidth = (int)(textWidth / multiplier);
                    length = (int)(title.length() / multiplier);
                    if (length < 3) {
                        length = 0;
                        title = "";
                    } else {
                        title = title.substring(0, length - 2).concat("..");
                    }
                }
                if (length > 0) {
                    float hOffset = slice.getValue()/totalValue == 1 ? arcLength*0.75f-textWidth/2 : (arcLength - textWidth) / 2;
                    canvas.drawTextOnPath(title, p, hOffset, vOffset, mPaint);
                }
            }
